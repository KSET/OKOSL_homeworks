...
def map_reduce_words():
    ...
    reduce = Code("function(key, values) {"
                  "var reduce_value = { words: [] };"
                  "var words = [];"
                  "var word_counter = [];"
                  "values.forEach(function(value) {"
                  "     if (words.indexOf(value.word) > -1) {"
                  "         for (var i = 0; i < word_counter.length; i++) {"
                  "             if (word_counter[i].word === value.word) {"
                  "                 word_counter[i].count += 1;"
                  "             }"
                  "         }"
                  "     }"
                  "     else {"
                  "         words.push(value.word);"
                  "         word_counter.push({word: value.word, count: value.count});"
                  "     }"
                  "});"
                  "word_counter = word_counter.sort(function(a,b) {"
                  "     return b.count - a.count;});"
                  "reduce_value.words = word_counter.slice(0, 10);"
                  "return reduce_value;};")

    result = mongo.news.map_reduce(map=map, reduce=reduce, out="most_common_words_per_author")

    authors = [] 
    for author in result.find().sort('_id', DESCENDING):
        author['value'] = author['value']['words']
        print(author)
        authors.append(author)

    return render_template('mr_words.html', title="MapReduce Author Word Count", authors=authors)
